      ...
      <main className="p-2 md:p-6">
         <div className="flex justify-between w-full">
            ...
            <Link href={"/projects/my-little-shop/dashboard/products/new-product"} className="">
               <Button className="rounded-2xl bg-emerald-700 hover:bg-emerald-600">
         ...
         <article className="w-full grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-4 lg:gap-8 mt-8 ">
            {
               products.map((p) => (
                  <section className="rounded-lg overflow-hidden font-poppins shadow-md  hover:shadow-xl cursor-pointer transition-all duration-300" key={p.id}>
                     ...
                     <img src={p.image} alt="Image dont load"  className="object-cover size-full"/>
                     </div>
                     <div className=" flex flex-col p-2 gap-2 text-xs">
                        <p className="text-md font-semibold truncate text-emerald-700 text-sm">{p.name}</p>
                        <p>Current stock: {p.currentStock}</p>
                        <p>Sale price: S/{p.salePrice}</p>

                     </div>
                     <div className="w-full flex justify-end">
                        <Link href={`products/detail?cod=${p.id}`}>
                        <Button className="mr-2 mb-2 bg-emerald-700 hover:bg-emerald-600" size={"sm"}>
                           DETAIL
                           <CircleArrowRight />
